    ...
    Database.findOne({ guildID: member.guild.id, userID: inviter.id }, (err, inviterData) => {
      if (!inviterData) {
        ...
        newInviter.save().then(x => {
          if (inviteChannel) inviteChannel.send(`${elmas} ${member} Sunucuya Katıldı, Davet eden kişi \`${inviter.id == member.guild.id ? member.guild.name : inviter}\`, Ulaştığı davet sayısı (\`${(x.regular ? x.regular : 0)+(x.bonus ? x.bonus : 0)}\`)`).catch(err => {});
        });
      } else {
        inviterData.fake++
        inviterData.save().then(x => {
          if (inviteChannel) inviteChannel.send(`${elmas} ${member} Sunucuya Katıldı, Davet eden kişi \`${inviter.id == member.guild.id ? "Vanity Url" : inviter}\`,  Ulaştığı davet sayısı (\`${(x.regular ? x.regular : 0)+(x.bonus ? x.bonus : 0)}\`)`).catch(err => {});
        });
      };
    });
  } else {
