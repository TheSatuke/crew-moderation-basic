...
module.exports.execute = async (client, message, args) => {
    ...
    let penals = await PenalManager.getPenals({ User: victim.id, Activity: true, $or: [{ Type: PenalManager.Types.TEMP_VOICE_MUTE }, {  Type: PenalManager.Types.TEMP_VOICE_MUTE  }] });
    if (penals.length <= 0) return message.lineReply(`${Settings.emojiler.iptal} ${victim} Bu kullanıcının hiç **Voice Mute** cezası yok.`);
    ...
    if (member && member.roles.cache.has(Settings.Penals.VoiceMute.Role)) member.roles.remove(Settings.Penals.VoiceMute.Role);

    message.lineReply(`${Settings.emojiler.tik} ${member} kullanıcısının \`#${penalId}\` Numaralı **Voice Mute** cezası kaldırıldı.`);
    message.react(Settings.emojiler.onayID)
    }
    else message.lineReply(`${Settings.emojiler.iptal} ${victim} **Voice Mute** cezası kaldırılma işlemi iptal edildi.`);


};

module.exports.settings = {
    Commands: ["unvoicemute","unvmute","uvmute"],
