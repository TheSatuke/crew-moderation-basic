...
module.exports.execute = async (client, message, args) => {
  ...
  if (!member) return message.channel.send(embed.setDescription(`${Settings.emojiler.iptal} Geçerli bir \`@Satuke/ID\` belirtmelisin`)).then(x => x.delete({ timeout: 5500 }));
  if (message.member.roles.highest.position <= member.roles.highest.position) return message.lineReply(embed
    .setDescription(`${Settings.emojiler.iptal} ${member} Bu işlemi gerçekleştiremiyorum (üst/aynı) yetkidesiniz`)).then(x => x.delete({ timeout: 5500 }));
    if (args[1]) {
      var newName;
      ...
      if (Config.name_age) {
        ...
        if (!name) return message.lineReply(embed.setDescription(`${Settings.emojiler.iptal} Geçerli bir isim belirtmelisin!`)).then(x => x.delete({ timeout: 5000 }));
        newName = `${name} Shiva`;
      };
...
module.exports.settings = {
  Commands: ["E", "man","erkek","ERKEK","Erkek","e"],
  Usage: "erkek",
  Description: "",
  Category: "General",
  Activity: true
